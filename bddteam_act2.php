...
if($super_count > $attr[$category]["max_super"]){
...
if($result = $sth->fetchObject()){
    ...
    remote_log("[報名失敗] 隊伍名稱已有人使用 $category-$teamname");
    echo json_encode($data);
    exit();
}
...
//insert team
$sql = "insert into teams(uid,category,teamname,reg_player,reg_stage) values(?,?,?,?,2)";
$sth = $db->prepare($sql);
...
if($pic_error!=""||$data->error!=""){
    ...
    rrmdir($thumb_dir);
$data->error="新增隊伍失敗\n".$data->error."\n".$pic_error;
remote_log("[報名失敗] 照片有問題 $category-$teamname uid=$uid");
}else{
    $data->redirect="../viewteam?tid=".$tid;
    //logging
    remote_log("[報名成功] $category-$teamname 報名成功");
